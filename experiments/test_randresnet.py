...
from src.training.solver_ridge import SolverRidge
from src.networks.randresnet import randResNet
...
parser.add_argument('--n_filters', type=int, default=64, help='number of filters (default: 64)')
parser.add_argument('--n_layers', type=int, default=2, help='number of layers (default: 2)')
parser.add_argument('--scaling', type=float, default=0.1, help='weight scaling for uniform init (default: 0.1)')
parser.add_argument('--alpha', type=float, default=1.0, help='residual branch coefficient (default: 1.0)')
parser.add_argument('--beta', type=float, default=1.0, help='non-linear branch coefficient (default: 1.0)')
parser.add_argument('--reg', type=float, default=0.0, help='regularization coefficient (default: 0.0)')
...
def main() -> None:
    ...
    if args.dataset == 'mnist':
        full_train_dataloader, _, val_dataloader, test_dataloader = data_utils.get_mnist(batch_size=args.batch_size)
    elif args.dataset == 'fmnist':
        full_train_dataloader, _, val_dataloader, test_dataloader = data_utils.get_fmnist(batch_size=args.batch_size)
    elif args.dataset == 'svhn':
        full_train_dataloader, _, val_dataloader, test_dataloader = data_utils.get_svhn(batch_size=args.batch_size)
    elif args.dataset == 'cifar10':
        full_train_dataloader, _, val_dataloader, test_dataloader = data_utils.get_cifar10(batch_size=args.batch_size)
    else:
        raise ValueError(
    ...
    for i in range(args.n_trials):
        ...
        results = solver.train(show_progress=args.show_progress)
    ...
    n_params = solver.classifier.coef_.size + solver.classifier.intercept_.size
    print(
        '##################################################\n'
